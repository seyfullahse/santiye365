...
export default function IsverenHakedisPage() {
  const router = useRouter();
  const { contracts: allContracts, selectedContractId, selectedContract: ctxContract } = useSozlesme();
  const contracts = allContracts.filter((c) => c.type === "ISVEREN");
  ...
  // Aktif sekme: "hakedisler" | "yesilDefter"
  const [activeTab, setActiveTab] = useState<"hakedisler" | "yesilDefter">("yesilDefter");

  // Derived
  const effectiveContractId = ctxContract?.type === "ISVEREN" ? selectedContractId : "";
  ...
  // Sözleşme bazlı tüm kalemler (ataşmansız olanlar dahil) filtrelenmiş
  ...
  // Totals (sadece ataşmanı olan kalemler)
  ...
  const handleCreate = async () => {
    if (!effectiveContractId) {
      toast.error("Lütfen sağ üstten bir İşveren sözleşmesi seçin");
      return;
    }
    try {
    ...
    <div className="space-y-6">
      ...
      {!effectiveContractId ? (
        <Card className="border-dashed">
          <CardContent className="py-8 text-center text-muted-foreground">
            ...
            <p className="text-sm mt-1">Sağ üstteki sözleşme seçiciden bir <strong>İşveren</strong> sözleşmesi seçin</p>
          </CardContent>
        </Card>
      ) : (
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <Card>
              <CardContent className="py-2.5 px-4">
                <div className="flex items-center gap-2">
                  <div className="rounded-md bg-green-100 p-1.5"><TrendingUp className="h-4 w-4 text-green-600" /></div>
                  <div>
                    ...
                    <p className="text-sm font-bold text-green-600 leading-tight mt-0.5">{fmtMoney(toplamHakedis, currency)}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="py-2.5 px-4">
                <div className="flex items-center gap-2">
                  <div className="rounded-md bg-orange-100 p-1.5"><Calculator className="h-4 w-4 text-orange-600" /></div>
                  <div>
                    <p className="text-[11px] text-muted-foreground leading-none">Son Dönem <span className="text-[10px]">({hakedisler.length} hak.)</span></p>
                    <p className="text-sm font-bold text-orange-600 leading-tight mt-0.5">{fmtMoney(sonDonemTutar, currency)}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          {activeTab === "yesilDefter" && (
            <>
              {/* Fiyat alt toplamları (detaylı modda) */}
                    ...
                    <CardContent className="py-2.5 px-4">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-bold text-emerald-700 leading-tight mt-0.5">{fmtMoney(totalGenel, currency)}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {Object.entries(grouped).map(([groupName, rows]) => {
                            ...
                            const colSpanBase = 7; // POZ ~ B.Fiyat
                                ...
                                >
                                  <TableCell colSpan={colSpanBase} className="font-semibold text-sm">
                                    <span className="flex items-center gap-2">
                                      ...
                                      {gAtas > 0 && (
                                        <Badge variant="outline" className="text-[10px] text-emerald-600">{gAtas} ataşmanlı</Badge>
                                      )}
                                    </span>
                                  </TableCell>
                                  {isDetailed && (
                              ...
                              <TableCell className="text-right">
                                <div className="flex justify-end gap-1">
                                  <Button
                                    ...
                                    variant="ghost"
                                    className="h-7 w-7"
                                    onClick={() => router.push(`/hakedis/isveren/${h.id}`)}
                                    title="Detay"
                                  >
      ...
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Yeni Hakediş Oluştur</DialogTitle>
          </DialogHeader>
