...
export default function HakedisDetayPage() {
  ...
  const handleDeleteAtasman = async (atasmanId: string) => {
    ...
    try {
      // Doğrudan atasman tablosundan sil
  ...
  const handleDeleteIhzarat = async (ihzaratId: string) => {
    ...
    try {
      // İhzarat silme - doğrudan prisma ile
  ...
  // Filtrelenmiş arama sonuçları (eklenmemiş olanlar)
  ...
  const handleExportAtasmanlar = () => {
    ...
    writeFileXLSX(wb, `HAK-${hakedis.no}-atasmanlar.xlsx`);
  ...
  const handleImportAtasmanlar = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Satırları ATŞ No + Kat/Bölge bazında grupla
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-6">
          <TabsTrigger value="ozet" className="text-xs sm:text-sm">
            Özet
          </TabsTrigger>
          <TabsTrigger value="atasmanlar" className="text-xs sm:text-sm">
            Ataşmanlar
          </TabsTrigger>
          <TabsTrigger value="ihzarat" className="text-xs sm:text-sm">
            İhzarat
          </TabsTrigger>
          <TabsTrigger value="yesil-defter" className="text-xs sm:text-sm">
            Yeşil Defter
          </TabsTrigger>
          <TabsTrigger value="mahsup" className="text-xs sm:text-sm">
            Mahsup
          </TabsTrigger>
          <TabsTrigger value="icmal" className="text-xs sm:text-sm">
            İcmal
          </TabsTrigger>
        </TabsList>
            ...
            <Card>
              <CardContent className="pt-4">
                ...
                <p className="text-lg font-bold font-mono">
                  {fmt(hakedis.contract?.totalAmount ?? 0)}
                </p>
              </CardContent>
            </Card>
            <Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span>
                    {hakedis.startDate
                      ? new Date(hakedis.startDate).toLocaleDateString("tr-TR")
                      : "—"}{" "}
                    –{" "}
                    {hakedis.endDate
                      ? new Date(hakedis.endDate).toLocaleDateString("tr-TR")
                      : "—"}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <CardContent className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono">
                      {fmt(toplamlar.icmal.yesilDefterToplam)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">İhzarat</span>
                    <span className="font-mono">
                      {fmt(toplamlar.icmal.ihzaratToplam)}
                    </span>
                  </div>
                  <div className="flex justify-between text-red-600">
                    <span>İhzarat Mahsubu</span>
                    <span className="font-mono">
                      - {fmt(toplamlar.icmal.mahsupToplam)}
                    </span>
                  </div>
                  <Separator />
                  <div className="flex justify-between font-semibold">
                    ...
                    <span className="font-mono">
                      {fmt(toplamlar.icmal.brutTutar)}
                    </span>
                  </div>
                  <div className="flex justify-between text-red-600">
                    <span>Toplam Kesinti</span>
                    <span className="font-mono">
                      - {fmt(toplamlar.icmal.kesintiler.toplamKesinti)}
                    </span>
                  </div>
                  <Separator />
        ...
        <TabsContent value="atasmanlar" className="space-y-4 mt-4">
          ...
          <input
            ...
            accept=".xlsx,.xls"
            className="hidden"
            onChange={handleImportAtasmanlar}
          />

          {hakedis.atasmanlar.length === 0 ? (
            <Card>
              <CardContent className="py-10 text-center text-muted-foreground">
                Henüz ataşman eklenmemiş. Saha ölçümlerini eklemek için
                &quot;Yeni Ataşman&quot; butonunu kullanın.
              </CardContent>
            </Card>
          ) : (
            <Card>
                    ...
                    <TableBody>
                            ...
                            {kIdx === 0 ? (
                              <TableCell rowSpan={a.kalemler.length} className="align-top">
                                <Button
                                  ...
                                  onClick={() => handleDeleteAtasman(a.id)}
                                >
                      ...
                      <TableRow className="bg-muted/50 font-bold">
                        ...
                        <TableCell className="text-right font-mono">
                          {fmtNum(
                            hakedis.atasmanlar.reduce(
                              (sum, a) => sum + a.kalemler.reduce((s, k) => s + k.miktar, 0),
                              0
                            )
                          )}
                        </TableCell>
        ...
        <TabsContent value="ihzarat" className="space-y-4 mt-4">
          ...
          {hakedis.ihzaratlar.length === 0 ? (
            <Card>
              <CardContent className="py-10 text-center text-muted-foreground">
                Henüz ihzarat eklenmemiş. Sahada stokta bulunan malzeme
                miktarlarını ekleyin.
              </CardContent>
            </Card>
          ) : (
                  ...
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                      <div
                        ...
                        onClick={() =>
                          setExpandedIhzarat(
                            expandedIhzarat === ihz.id ? null : ihz.id
                          )
                        }
                      >
                        ...
                        <span className="text-xs text-muted-foreground">
                          ({ihz.kalemler.length} kalem)
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        {ihz.tarih && (
                          <span className="text-xs text-muted-foreground">
                            {new Date(ihz.tarih).toLocaleDateString("tr-TR")}
                          </span>
                        )}
                        <Button
                          size="icon"
                          variant="ghost"
                          className="h-7 w-7 text-red-500"
                          onClick={() => handleDeleteIhzarat(ihz.id)}
                        >
                          <Trash2 className="h-3.5 w-3.5" />
                        </Button>
                      </div>
                          ...
                          {ihz.kalemler.map((k) => (
                            <TableRow key={k.id}>
                              ...
                              <TableCell className="text-right font-mono text-xs">
                                {fmt(k.kesifKalemi.malzemeFiyati)}
                              </TableCell>
                              <TableCell className="text-right font-mono text-xs font-semibold">
                                {fmt(k.miktar * k.kesifKalemi.malzemeFiyati)}
                              </TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {toplamlar.yesilDefter
                        .filter(
                          (y) =>
                            y.kumulatifMiktar > 0 || y.buDonemMiktar > 0
                        )
                        .map((y) => (
                          <TableRow key={y.kesifKalemiId}>
                            ...
                            <TableCell className="text-right font-mono text-xs">
                              {fmtNum(y.sozlesmeMiktar)}
                            </TableCell>
                            <TableCell className="text-right font-mono text-xs">
                              {fmtNum(y.oncekiMiktar)}
                            </TableCell>
                            <TableCell className="text-right font-mono text-xs font-semibold text-green-600">
                              {fmtNum(y.buDonemMiktar)}
                            </TableCell>
                            <TableCell className="text-right font-mono text-xs">
                              {fmtNum(y.kumulatifMiktar)}
                            </TableCell>
                            <TableCell className="text-right font-mono text-xs font-semibold">
                              {fmt(y.buDonemTutar)}
                            </TableCell>
                            <TableCell className="text-right font-mono text-xs">
                              {fmt(y.kumulatifTutar)}
                            </TableCell>
                          </TableRow>
                        ))}
                      {/* Toplam satırı */}
                      <TableRow className="bg-muted/50 font-bold">
                        ...
                        <TableCell className="text-right font-mono">
                          {fmt(toplamlar.icmal.yesilDefterKumulatif)}
                        </TableCell>
                      </TableRow>
                    </TableBody>
                  </Table>
        ...
        <TabsContent value="mahsup" className="space-y-4 mt-4">
          ...
          <p className="text-sm text-muted-foreground">
            İmalat yapıldığında, daha önce ihzarat olarak ödenen malzeme
            bedelinin düşülmesi. Mahsup = min(Küm. İmalat, Küm. İhzarat) ×
            Malzeme Fiyatı − Önceki Mahsup
          </p>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {toplamlar.ihzaratMahsubu
                        .filter(
                          (m) =>
                            m.kumulatifIhzarat > 0 || m.kumulatifImalat > 0
                        )
                        .map((m) => (
                          <TableRow key={m.kesifKalemiId}>
                            ...
                            <TableCell className="text-right font-mono text-xs">
                              {fmtNum(m.kumulatifImalat)}
                            </TableCell>
                            <TableCell className="text-right font-mono text-xs">
                              {fmtNum(m.kumulatifIhzarat)}
                            </TableCell>
                            <TableCell className="text-right font-mono text-xs">
                              {fmtNum(m.oncekiMahsupMiktar)}
                            </TableCell>
                            <TableCell className="text-right font-mono text-xs font-semibold text-orange-600">
                              {fmtNum(m.buDonemMahsupMiktar)}
                            </TableCell>
                            <TableCell className="text-right font-mono text-xs font-semibold">
                              {fmt(m.buDonemMahsupTutar)}
                            </TableCell>
                          </TableRow>
                        ))}
                      <TableRow className="bg-muted/50 font-bold">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <CardContent className="space-y-3 text-sm">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-red-600">
                      ...
                      <span className="font-mono">
                        − {fmt(toplamlar.icmal.kesintiler.avansKesintisi)}
                      </span>
                    </div>
                    <div className="flex justify-between text-red-600">
                      <span>Teminat Kesintisi</span>
                      <span className="font-mono">
                        − {fmt(toplamlar.icmal.kesintiler.teminatKesintisi)}
                      </span>
                    </div>
                    <div className="flex justify-between text-red-600">
                      <span>Damga Vergisi</span>
                      <span className="font-mono">
                        − {fmt(toplamlar.icmal.kesintiler.damgaVergisi)}
                      </span>
                    </div>
                    <div className="flex justify-between text-red-600">
                      <span>Diğer Kesintiler</span>
                      <span className="font-mono">
                        − {fmt(toplamlar.icmal.kesintiler.digerKesintiler)}
                      </span>
                    </div>
                  </div>
                  <Separator />
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    HAK-1&apos;den HAK-{hakedis.no}&apos;e kadar toplam
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3 text-sm">
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono">
                      {fmt(toplamlar.icmal.ihzaratKumulatif)}
                    </span>
                  </div>
                  <div className="flex justify-between text-orange-600">
                    <span>Kümülatif Mahsup</span>
                    <span className="font-mono">
                      − {fmt(toplamlar.icmal.mahsupKumulatif)}
                    </span>
                  </div>
                  <Separator />
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono font-semibold">
                      ...
                      {hakedis.contract?.totalAmount
                        ? (
                            (toplamlar.icmal.yesilDefterKumulatif /
                              hakedis.contract.totalAmount) *
                            100
                          ).toFixed(1)
                        : "0"}
          ...
          <div className="space-y-4 mt-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  placeholder="Örn: B1 Kat, Blok A"
                  value={atasmanKatBolge}
                  onChange={(e) => setAtasmanKatBolge(e.target.value)}
                />
              </div>
              <div>
                <Label>Genel Açıklama</Label>
                <Input
                  placeholder="Ataşman genel açıklaması"
                  value={atasmanAciklama}
                  onChange={(e) => setAtasmanAciklama(e.target.value)}
                />
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mb-2">
                Poz numarası veya iş kalemi adı ile arayın, sonuçlardan seçin
              </p>
              ...
              {pozSearchFocused && pozSearchResults.length > 0 && (
                <div className="mt-1 border rounded-lg bg-popover shadow-md max-h-[240px] overflow-y-auto">
                  ...
                  {pozSearch.trim().length >= 1 && pozSearchResults.length === 12 && (
              ...
              {pozSearchFocused && pozSearch.trim().length >= 1 && pozSearchResults.length === 0 && (
                <div className="mt-1 border rounded-lg bg-popover shadow-md px-3 py-4 text-center">
                  <p className="text-xs text-muted-foreground">
                    &quot;{pozSearch}&quot; ile eşleşen poz bulunamadı
                  </p>
                </div>
              )}
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-sm font-semibold">
                  Seçilen Kalemler ({atasmanKalemler.length})
                </h3>
                {kesifKalemleri.length <= 50 && (
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (missing.length === 0) {
                        toast.info("Tüm pozlar zaten ekli");
                        return;
                      }
                      setAtasmanKalemler((prev) => [
                        ...prev,
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {atasmanKalemler.map((kalem, idx) => {
                        ...
                        return (
                          <TableRow key={kalem.kesifKalemiId} className={kalem.miktar > 0 ? "bg-green-50 dark:bg-green-950/20" : ""}>
                            ...
                            <TableCell>
                              <Button
                                ...
                                variant="ghost"
                                className="h-7 w-7 text-red-500"
                                onClick={() => removePozFromAtasman(kalem.kesifKalemiId)}
                              >
        ...
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-2">
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800">
              <strong>İhzarat:</strong> Sahada stokta bulunan ancak henüz imalatı
              yapılmamış malzeme miktarlarını girin. Malzeme ön ödemesi olarak
              hesaba katılacaktır.
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  type="date"
                  value={ihzaratTarih}
                  onChange={(e) => setIhzaratTarih(e.target.value)}
                />
              </div>
              <div>
                <Label>Açıklama</Label>
                <Input
                  placeholder="İhzarat açıklaması"
                  value={ihzaratAciklama}
                  onChange={(e) => setIhzaratAciklama(e.target.value)}
                />
              </div>
            </div>
            ...
            <div>
              <h3 className="text-sm font-semibold mb-2">
                Keşif Kalemleri — Stok Miktarlarını Girin
              </h3>
              <div className="border rounded-lg overflow-x-auto">
                <Table>
                  <TableHeader>
