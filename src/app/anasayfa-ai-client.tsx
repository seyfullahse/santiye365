...
/* ╔══════════════════════════════════════════════════════╗
   ║          KLASİK GÖRÜNÜM (CLASSIC VIEW)              ║
   ╚══════════════════════════════════════════════════════╝ */

const classicModules = [
  { name: "Yönetim Paneli", icon: LayoutDashboard, href: "/yonetim-paneli", color: "bg-orange-500", description: "Genel yönetim ve istatistikler" },
  { name: "Proje Yönetimi", icon: FolderKanban, href: "/dashboard", color: "bg-blue-600", description: "Projelerinizi yönetin" },
  { name: "Mahaller", icon: MapPin, href: "/mahaller", color: "bg-emerald-500", description: "Mahal tanımlama ve takip" },
  { name: "Katlar", icon: Layers, href: "/katlar", color: "bg-teal-500", description: "Kat bazlı ilerleme" },
  { name: "Aktiviteler", icon: Activity, href: "/aktiviteler", color: "bg-green-500", description: "İş kalemleri takibi" },
  { name: "Onaylar", icon: CheckCircle2, href: "/onaylar", color: "bg-amber-500", description: "Onay süreçleri" },
  { name: "Riskler", icon: AlertTriangle, href: "/riskler", color: "bg-red-500", description: "Risk yönetimi" },
  { name: "Hakediş", icon: FileText, href: "/hakedis", color: "bg-purple-500", description: "Hakediş ve kesintiler" },
  { name: "Malzeme Takip", icon: Package, href: "/malzemeler", color: "bg-violet-500", description: "Malzeme stok takibi" },
  { name: "Şirketler", icon: Building2, href: "/sirketler", color: "bg-rose-500", description: "Taşeron şirket yönetimi" },
  { name: "Ekipler", icon: Users, href: "/ekipler", color: "bg-pink-500", description: "Ekip yönetimi" },
  { name: "Çalışanlar", icon: HardHat, href: "/calisanlar", color: "bg-cyan-500", description: "Personel bilgileri" },
  { name: "Puantaj", icon: ClipboardList, href: "/puantaj", color: "bg-fuchsia-500", description: "Günlük devam takibi" },
  { name: "Ayarlar", icon: Settings, href: "/ayarlar", color: "bg-gray-500", description: "Sistem ayarları" },
];
...
function ClassicModuleCard({ mod }: { mod: (typeof classicModules)[number] }) {
  const Icon = mod.icon;
  return (
    <Link href={mod.href} className="block group">
      <Card className="h-full hover:shadow-lg transition-all duration-200 hover:scale-[1.02] border-muted">
        <CardContent className="pt-6">
          ...
          <div className="mt-3 flex items-center gap-1 text-xs text-muted-foreground/60 group-hover:text-primary transition-colors">
...
/* ╔══════════════════════════════════════════════════════╗
   ║      AI SUNUM GÖRÜNÜMÜ (CINEMATIC AI VIEW)          ║
   ╚══════════════════════════════════════════════════════╝ */
...
const aiSections = [
  {
    ...
    modules: [
      { key: "yonetim-paneli", name: "Yönetim Paneli", tagline: "Tüm projeler, tek ekran. Gerçek zamanlı karar destek sistemi.", icon: LayoutDashboard, href: "/yonetim-paneli", active: true, gradient: "from-orange-500 to-amber-500", bg: "bg-orange-500/10", text: "text-orange-500" },
        ...
        <div className="mx-auto max-w-6xl flex h-14 items-center justify-between px-4 sm:px-6">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="flex items-center gap-3">
            ...
            <Avatar className="h-8 w-8 border border-white/10">
              <AvatarFallback className="bg-white/5 text-white/60 text-xs font-semibold">{userName.charAt(0).toUpperCase()}</AvatarFallback>
            </Avatar>
            <button
              onClick={() => signOut({ callbackUrl: "/giris" })}
                ...
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <Badge className="bg-violet-500/10 text-violet-400 border-violet-500/20 text-[9px] font-semibold">GPT-4o</Badge>
                    </div>
            ...
            <div className="relative overflow-hidden rounded-2xl border border-violet-500/20 bg-gradient-to-br from-violet-500/5 to-purple-500/5 backdrop-blur-sm">
              ...
              <div className="relative p-8 sm:p-12 text-center">
                ...
                <p className="text-white/40 text-sm max-w-md mx-auto mb-6">
                  Verilerinizi okur, öngörüler sunar, kararlarınızı hızlandırır.<br />7/24 dijital danışman.
                </p>
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1 }} className="text-center pb-8">
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-3">
              <Link href="/yonetim-paneli">
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-cyan-500 text-white text-sm font-medium shadow-lg shadow-blue-500/20 hover:shadow-blue-500/40 transition-shadow">
                  <BarChart3 className="h-4 w-4" />Yönetim Paneline Git
                </motion.button>
              </Link>
              <Link href="/dashboard">
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl border border-white/10 bg-white/5 text-white/80 text-sm font-medium hover:bg-white/10 transition-colors">
                  <FolderKanban className="h-4 w-4" />Proje Yönetimine Git
                </motion.button>
              </Link>
            </div>
          </motion.div>
...
function AnasayfaContent(props: AnasayfaProps) {
  const [viewMode, setViewMode] = useState<"classic" | "ai">("ai");
